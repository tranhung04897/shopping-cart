  ...
  showCartItem= (cart) => {
    ...
    if(cart.length > 0) {
      result = cart.map((cartItem,index)=>{
        return <CartItem key={index} cartItem={cartItem.product} quantity={cartItem.quantity} onDeleteProductInCart={onDeleteProductInCart} onChangeMessage={onChangeMessage} />
      })
    }
    return result;
  }
